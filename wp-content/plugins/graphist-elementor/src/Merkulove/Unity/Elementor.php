...
final class Elementor {
	...
	public function editor_styles() {
		...
		wp_enqueue_style( 'mdp-graphist-elementor-admin', Plugin::get_url() . 'src/Merkulove/Unity/assets/css/elementor-admin' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
	...
	public function register_widgets() {
		...
		$path = Plugin::get_path() . 'src/Merkulove/GraphistElementor/Elementor/widgets/';
		...
		foreach ( new RecursiveIteratorIterator( new RecursiveDirectoryIterator( $path ) ) as $filename ) {
			...
			if ( substr( $filename, -4 ) === '.php' ) {
				...
				/** Add Namespace. */
                $widget_class = 'Merkulove\GraphistElementor\\' . $widget_class;
