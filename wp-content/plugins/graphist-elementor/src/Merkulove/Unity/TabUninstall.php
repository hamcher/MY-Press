...
final class TabUninstall extends Tab {
	...
	public function add_settings() {
		...
		/** Delete plugin. */
		add_settings_field( 'delete_plugin', esc_html__( 'Removal settings:', 'graphist-elementor' ), [$this, 'render_delete_plugin'], $group, $section );
	...
	public function render_delete_plugin() {
		...
		$uninstall_settings = get_option( 'mdp_graphist_elementor_uninstall_settings' );
		...
		/** Prepare description. */
		$helper_text = esc_html__( 'Choose which data to delete upon using the "Delete" action in the "Plugins" admin page.', 'graphist-elementor' );

		/** Render select. */
		UI::get_instance()->render_select(
			$options,
			$uninstall_settings[ 'delete_plugin' ], // Selected option.
			esc_html__( 'Delete plugin', 'graphist-elementor' ),
			$helper_text,
			[
			    'name' => 'mdp_graphist_elementor_uninstall_settings[delete_plugin]'
            ]
		);
